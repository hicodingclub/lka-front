import { Input, Component } from '@angular/core';

import { MusergroupComponent } from '../../../access/musergroup/musergroup.component';

@Component({
    template: '',
})
export class MusergroupListCustComponent extends MusergroupComponent {
}

/*>>> Please check this recent updates and merge with existing ones***
**Date: Tue Jul 07 2020 10:25:06 GMT-0700 (Pacific Daylight Time)

import {
  Input,
  Component
} from '@angular/core';
import {
  MusergroupComponent
} from '../../../access/musergroup/musergroup.component';
@Component({
  template: '',
})
export class MusergroupListCustComponent extends MusergroupComponent {}**** End of recent updates.<<<*/
